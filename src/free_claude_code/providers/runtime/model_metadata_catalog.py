...
_MODELS_DEV_PROVIDER_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    # Keep B.AI distinct from the unrelated ``bailing`` provider in models.dev.
    "bai": ("bai",),
...
class ModelMetadataCatalog:
    ...
    def _enrich_info(
        ...
        pricing = info.pricing or metadata.pricing
        return replace(
            info,
...
def _metadata_from_entry(
    ...
    *,
    ...
    return ModelCatalogMetadata(
        ...
        is_free=_bool_value(entry.get("is_free")),
        pricing=normalize_model_pricing(
            entry.get("cost") if entry.get("cost") is not None else entry.get("pricing")
        ),
...
def _metadata_from_mapping(
    ...
    *,
    ...
    return ModelCatalogMetadata(
        ...
        is_free=_bool_value(value.get("is_free")),
        pricing=normalize_model_pricing(value.get("pricing")),
