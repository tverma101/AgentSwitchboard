...
| --- | --- |
| Command palette (`cmd+p` / `ctrl+k`) | `Ctrl+K` / `Ctrl+P` palette in the Rust control center covering all 10 pages plus every page-contextual action (`src/free_claude_code/native_tui/src/app.rs`: `palette_inventory`, `match_palette`) |
