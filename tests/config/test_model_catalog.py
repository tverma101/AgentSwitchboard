...
def test_settings_accept_generic_catalog_values() -> None:
    settings = Settings(
        **{
    ...
    assert settings.model_catalog_mode is ModelCatalogMode.CURATED
    assert settings.model_catalog_allowlist == "opencode_go/*"


def test_empty_generic_mode_is_legacy_compatibility_sentinel() -> None:
    settings = Settings.model_validate({"MODEL_CATALOG_MODE": ""})

    assert settings.model_catalog_mode is None
