...
async def test_local_a3s_success_precedes_hosted_backends() -> None:
    local_results = [{"title": "Local result", "url": "https://example.com/local"}]
    ...
    with (
        ...
        results = await _run_web_search("capital of France", use_local_a3s=True)
...
async def test_local_a3s_failure_falls_back_to_firecrawl() -> None:
    firecrawl_results = [
        {"title": "Hosted result", "url": "https://example.com/hosted"}
    ]
